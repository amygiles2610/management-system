...
class Calendar:
    def __init__(self, master, values={}):
        ...
        self.frame.place(relx=0.5, rely=0.5, anchor=E,x=-90,y=10)

        self.frame1=Frame(master,width=300, height=200, bg='white')
        self.frame1.place(relx=0.5, rely=0.5, anchor=W,x=-20,y=-20)

        def new_event_popup():
            subprocess.call("NewEvent.py", shell=True)
            
        self.new_event_window = Button(master, text='Add Event', command=new_event_popup)
        self.new_event_window.place(relx=0.5, rely=0.5, anchor=W,x=200,y=100)
    ...
    def setup(self, y, m):
        ...
        self.sel = Label(self.frame1, height=2, bg='white',fg='black',text='')
        self.wid.append(self.sel)
        self.sel.place(x=2, y=5)
            ...
            for d, day in enumerate(week):
                if day:
                    if self.day == day and self.month == datetime.date.today().month and self.year==datetime.date.today().year:
                        self.sel.config(text='')
                        self.sel.config(text='{} {} {} {}'.format(self.day_name, calendar.month_name[self.month_selected], self.day_selected, self.year_selected))
